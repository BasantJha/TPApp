...
//
class EmployerAadhaar_Verification extends StatefulWidget {

  const EmployerAadhaar_Verification({Key? key, this.liveModelObj, this.employerAadharArriveFrom,}) : super(key: key);
  final Employer_VerifyMobileNoModelClass? liveModelObj;
  final String? employerAadharArriveFrom;

  @override
  State<EmployerAadhaar_Verification> createState() => _EmployerAadhaar_Verification();
}

class _EmployerAadhaar_Verification extends State<EmployerAadhaar_Verification>
{
  ...
  Employer_AadhaarSendOTPModelClass? aadhaarSendOTPModelClass;
  String aadhaarOTP="";
                                    ...
                                    child: Container(
                                      ...
                                      child: TextButton(
                                        onPressed: ()
                                        {
                                          ...
                                          createBodyWebApi_SendAadhaarOTPForEmployer();
  ...
  sendAadhaarOTP()
  {
    ...
    {
      createBodyWebApi_VerifyAadhaarOTPForEmployer();
    }
    else
    {
  ...
  createBodyWebApi_SendAadhaarOTPForEmployer()
  {//
    var mapObject=getEmployer_KYC_AadharSendOTP_RequestBody(widget.liveModelObj?.employerId!,completeAadhaarNo);
    serviceRequestForSendAadhaarOTPEmployer(mapObject);
  }
  serviceRequestForSendAadhaarOTPEmployer(Map mapObj)
  {
    ...
    CJEmployerServiceRequest().postDataServiceRequest(mapObj,JG_ApiMethod_AadhaarSendOTP,
        cjEmployerResponseBlock: CJEmployerResponseBlock(employerSuccessBlock: <T>(commonResponse,modelResponse)
        {
          EasyLoading.dismiss();
          print("show sucess");

          aadhaarSendOTPModelClass=modelResponse as Employer_AadhaarSendOTPModelClass;
        ...
        async {
          ...
          CJTalentCommonModelClass commonModelClass=commonResponse as CJTalentCommonModelClass;
          String? message=commonModelClass?.message;

          if (message==null || message=="")
          {
            CJSnackBar(context, "Oops, something went wrong. Please try again latter.");
          }else {

            /*----------17-2-2023 START--------*/

            if(commonModelClass.aadharVerificationStatus==TankhaPay_Aadhaar_NR)
            {
              await EmployerAadharNotResponding().showBottomSheetAadharSkipMethod(context,message!,widget.liveModelObj,widget.employerAadharArriveFrom!);
            }
            else
            {
  ...
  createBodyWebApi_VerifyAadhaarOTPForEmployer()
  {
    var aadhaarClientId=aadhaarSendOTPModelClass!.clientId;

    var mapObject=getEmployer_KYC_AadharVerifyOTP_RequestBody(aadhaarClientId!,aadhaarOTP,widget.liveModelObj?.employerId!,widget.liveModelObj?.employerMobile!);
    serviceRequestForVerifyAadhaarOTPEmployer(mapObject);

  }
  serviceRequestForVerifyAadhaarOTPEmployer(Map mapObj)
  {
    ...
    CJEmployerServiceRequest().postDataServiceRequest(mapObj,JG_ApiMethod_AadhaarVerifyOTP,
        cjEmployerResponseBlock: CJEmployerResponseBlock(employerSuccessBlock: <T>(commonResponse,modelResponse)
        {
          EasyLoading.dismiss();
          print("show sucess");

          EmployerAadhaarSendOTPModelClass aadhaarSendOTPModelClass=modelResponse as EmployerAadhaarSendOTPModelClass;
          if(aadhaarSendOTPModelClass.statusCode==true)
          {
            print("show sucess1");
            CJSnackBar(context, aadhaarSendOTPModelClass.message!);


              /*-----------17-2-2023 start-------------*/
            if(widget.employerAadharArriveFrom==TankhaPay_EmployerHome)
              {
                Navigator.pop(context);
              }
            else
        ...
        async {
          ...
          CJTalentCommonModelClass commonModelClass=commonResponse as CJTalentCommonModelClass;
          String? message=commonModelClass?.message;
          print("show the verify message ${commonModelClass.message}");
          if (message==null || message=="")
          {
            CJSnackBar(context, "Oops, something went wrong. Please try again latter.");
          }else
          {
            ...
            if(widget.employerAadharArriveFrom=="" || widget.employerAadharArriveFrom==null)
            {
              userArriveFrom="";
            }
            else
            {
              userArriveFrom=widget.employerAadharArriveFrom;
            }

            if(commonModelClass.aadharVerificationStatus==TankhaPay_Aadhaar_NR)
            {
              await EmployerAadharNotResponding().showBottomSheetAadharSkipMethod(context,message,widget.liveModelObj,userArriveFrom!);
            }
            else
            {
              CJSnackBar(context,message);
...
class EmployerAadharNotResponding
{
  Future<Future> showBottomSheetAadharSkipMethod(BuildContext context,String message,Employer_VerifyMobileNoModelClass? liveModelObj,String employerAadharArriveFrom)
  async {
                          ...
                          InkWell(onTap: ()
                            {
                              ...
                              /*-----------17-2-2023 start-------------*/
                              if(employerAadharArriveFrom==TankhaPay_EmployerHome)
                              {
                                Navigator.pop(context);
                              }
                              else
                              {
                                TalentNavigation().pushTo(context, Employer_JoinerHome(liveModelObj: liveModelObj,));
                              }
                              /*-----------17-2-2023 end-------------*/
//
